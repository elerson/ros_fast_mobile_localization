...
class Robot:
    def __init__(self, odom_alphas):
        ...
        while(not (self.start_map and self.start_grounf_truth)):
            time.sleep(0.1)
        t = 15
        self.change_direction_time = [t,2*t,3*t,4*t]
        self.time = time.time()
...
if __name__ == "__main__":
    ...
    exp_file = rospy.get_param("~exp_file")
    ...
    odom_alphas = exp_config['odom_alphas']
    ...
    localization_algs = []
    for algorithm in exp_config['algorithm']:           
        for sensor_set in exp_config['sensor_set']:
            ...
            for sensor in sensor_set:
                ...
                elif(sensor == 'Decawave'):
                    if(Decawave_ == None):
                        Decawave_ = Decawave(robot, config_file)
                    sensor_ = Decawave_

                elif(sensor == 'DoubleSensors'):
                    if(DoubleSensors_ == None):
                        DoubleSensors_ = DoubleSensors(robot, config_file)
                    sensor_ = DoubleSensors_


                elif(sensor == 'Sensors'):
                    if(Sensors_ == None):
                        Sensors_ = Sensors(robot, config_file)

                    sensor_ = Sensors_


                elif(sensor == 'DoubleDecawave'):
                    if(DoubleDecawave_ == None):
                        DoubleDecawave_ = DoubleDecawave(robot, config_file)
    ...
    while not rospy.is_shutdown():
        ...
        if(robot.isEnded()):
            name = '/home/elerson/.ros/test_square_'+str(time.time())[:-3]
